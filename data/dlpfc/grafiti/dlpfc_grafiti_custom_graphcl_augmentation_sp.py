...
for iteration, filename in enumerate(os.listdir(f'{datadir}/raw')):
    ...
    if os.path.isfile(file_path):
        ...
        gae = GAE(adata, layers=[50,50], lr=0.0001, device=device, aug_type='subgraph', alpha=10, beta=1, gamma=.995)#, exponent=2, distance_scale=10)
        ...
        adata.write(f'{datadir}/grafiti/{filename[:6]}_grafiti_gcl_{gae.aug_type}_{gae.encoder_layers[0]}_{gae.encoder_layers[1]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.h5ad')

with open(f'{datadir}/grafiti/ari_grafiti_gcl_{gae.aug_type}_{gae.encoder_layers[0]}_{gae.encoder_layers[1]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.h5ad', 'wb') as f:
