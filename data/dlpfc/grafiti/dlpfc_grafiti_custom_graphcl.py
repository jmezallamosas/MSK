...
class GAE(object):
    ...
    def train(self, epochs, update_interval=5, threshold=0.001, patience=10):
        ...
        for i in range(epochs):
            ...
            s2 = self.read(h2, to_dense_adj(self.data.edge_index2)[0].fill_diagonal_(1)).to(self.device)
...
for filename in os.listdir(f'{datadir}/raw'):
    ...
    if os.path.isfile(file_path):
        ...
        gae = GAE(adata, layers=[50,50], lr=0.0001, device=device, alpha=1, beta=10, gamma=.995)#, exponent=2, distance_scale=10)
        ...
        adata.write(f'{datadir}/grafiti/{filename[:6]}_grafiti_gcl_{gae.encoder_layers[0]}_{gae.encoder_layers[1]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.h5ad')

with open(f'{datadir}/grafiti/ari_grafiti_gcl_{gae.encoder_layers[0]}_{gae.encoder_layers[1]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.h5ad', 'wb') as f:
