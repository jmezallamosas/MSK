...
for filename in os.listdir(f'{datadir}/raw'):
    ...
    if os.path.isfile(file_path):
        ...
        gae = GAE(adata, layers=[64], lr=0.0001, device=device, alpha=5, beta=10, gamma=1)#, exponent=2, distance_scale=10)
        ...
        adata.write(f'{datadir}/grafiti/{filename[:6]}_grafiti_lodcl_pca_{gae.encoder_layers[0]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.h5ad')

with open(f'{datadir}/grafiti/ari_grafiti_lodcl_pca_{gae.encoder_layers[0]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.pkl', 'wb') as f:
