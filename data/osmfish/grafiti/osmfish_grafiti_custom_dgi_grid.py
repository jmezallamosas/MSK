...
for i in range(10):
    ...
    gae = GAE(adata, layers=[100,100], lr=0.0001, device=device, alpha=1, beta=10, gamma=.995)#, exponent=2, distance_scale=10)
    ...
    adata.write(f'{datadir}/grafiti/{i+1}_grafiti_lodcl_grid_{gae.encoder_layers[0]}_{gae.encoder_layers[1]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.h5adf')

with open(f'{datadir}/grafiti/ari_grafiti_lodcl_grid_{gae.encoder_layers[0]}_{gae.encoder_layers[1]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.h5ad', 'wb') as f:
