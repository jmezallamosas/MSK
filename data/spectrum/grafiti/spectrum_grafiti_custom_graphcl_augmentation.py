...
gae = GAE(new_adata, layers=[20,20], lr=0.001, device=device, aug_type='node', alpha=5, beta=5, gamma=1)#, exponent=2, distance_scale=10)
print(f'{gae.aug_type},{gae.encoder_layers[0]},{gae.encoder_layers[1]},{gae.lr},{gae.alpha},{gae.beta},{gae.gamma}')
gae.train(10000, update_interval=100, threshold=1e-3, patience=10)
gae.load_embedding(new_adata, encoding_key="X_grafiti") # Load features into the z latent space
gf.tl.umap(new_adata) # Embed grafiti latent space into umap latent space (2 dimensions)
gf.tl.find_motifs_gmm(new_adata,k=15)
new_adata.write(f'{datadir}/grafiti/spectrum_grafiti_gcl_{gae.aug_type}_{gae.encoder_layers[0]}_{gae.encoder_layers[1]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.h5ad')
