...
adata = sc.read_h5ad(f'{datadir}/raw/spectrum_bool.h5ad') # Boolean processed features according to phenotype_full in obs

#features = ['panCK', 'CD8', 'CD68', 'PD1', 'PDL1', 'TOX']
#new_adata = adata[:, features].copy()

gae = gf.ml.GAE(adata, layers=[10,10], lr=0.001, device=device, alpha=5, beta=10, gamma=0.995)#, exponent=2, distance_scale=10)
...
adata.write(f'{datadir}/grafiti/spectrum_grafiti_bool_dcl_{gae.encoder_layers[0]}_{gae.encoder_layers[1]}_{gae.lr}_{gae.alpha}_{gae.beta}_{gae.gamma}.h5ad')
